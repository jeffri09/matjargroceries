    ...
    return (
        <main className={styles.page}>
            <div className={styles.header}>
                ...
                <p className={styles.subtitle}>Search orders, select, and print driver receipts in batch</p>
            </div>
                            ...
                            <div className={styles.toolbar}>
                                ...
                                <div className={styles.actionGroup}>
                                    <button className={styles.selectAllBtn} onClick={toggleAll}>
                                        ...
                                        {selected.size === filteredOrders.length ? 'Deselect All' : 'Select All'}
                                    </button>
                                    {selected.size > 0 && (
                                        <button className={styles.printBtn} onClick={handlePrint}>
                                            <span className="material-symbols-outlined" style={{ fontSize: '18px' }}>print</span>
                                            Print {selected.size} Receipt
                                        </button>
                                    )}
                                </div>
                            </div>
                                    ...
                                    >
                                        ...
                                        <div className={styles.orderInfo}>
                                            <div className={styles.orderTop}>
                                                ...
                                                <span className={styles.orderPay}>{order.metode_bayar}</span>
                                            </div>
                                            <p className={styles.orderName}>{order.nama} — {order.alamat}</p>
                    ...
                    return (
                        <div key={order.id_order} className={styles.printReceipt}>
                            <div className={styles.printReceiptHeader}>
                                <div>
                                    <h2 className={styles.printTitle}>🚚 Delivery Receipt</h2>
                                    <p className={styles.printBrand}>Matjar Groceries</p>
                                </div>
                            ...
                            <div className={styles.printSection}>
                                <h3 className={styles.printSectionTitle}>📦 Origin Store</h3>
                                <p>{store.nama} — {store.alamat}</p>
                                <p>Phone: {store.telepon}</p>
                            </div>
                            ...
                            <div className={styles.printSection}>
                                ...
                                <p>{order.alamat}</p>
                                <p>Phone: {order.telepon}</p>
                                {routeLink && <p>🗺️ Route: {routeLink}</p>}
                            </div>
                            ...
                            {/* Items */}
                            <div className={styles.printSection}>
                                <h3 className={styles.printSectionTitle}>🧺 Orders</h3>
                                {items.map((item, idx) => (
                                    <p key={idx}>
                            ...
                            <div className={styles.printSection}>
                                ...
                                <p>Pay: {order.metode_bayar}</p>
                                <p>Schedule: {order.jadwal}</p>
                                {order.catatan && <p>Note: {order.catatan}</p>}
                            </div>
                        </div>
                    );
                })}
