...
type ReceiptMode = 'pembeli' | 'kurir';
...
export default function ReceiptPage({ params }: { params: Promise<{ orderId: string }> }) {
    ...
    const [mode, setMode] = useState<ReceiptMode>('pembeli');
    ...
    const handleShare = () => {
        ...
        const text = `🧾 Matjar Groceries Receipt\nID: ${orderId}\n${url}`;
        ...
        return (
            <main className={styles.page}>
                <div className={styles.errorState}>
                    ...
                    <h2>Order Tidak Found</h2>
                    <p>{error || `Order ${orderId} tidak ditemukan.`}</p>
                    <div className={styles.errorActions}>
                        <Link href="/find-order" className={styles.btnSecondary}>
    ...
    return (
        <main className={styles.page}>
            ...
            <div className={styles.successHeader}>
                ...
                <p className={styles.successDesc}>Your order has been sent via WhatsApp</p>
            </div>

            {/* Mode Toggle — only show if admin (via ?mode=kurir) */}
            {isAdminMode && (
            ...
            <div className={styles.receipt} id="struk-cetak">
                ...
                <div className={styles.receiptHeader}>
                    <div>
                        <h2 className={styles.receiptTitle}>
                            {mode === 'kurir' ? '🚚 Delivery Receipt' : '🧾 Purchase Receipt'}
                        </h2>
                        <p className={styles.receiptBrand}>Matjar Groceries</p>
                    </div>
                        ...
                        <>
                            ...
                            {order.diskon > 0 && (
                                <div className={`${styles.summaryRow} ${styles.summaryDiscount}`}>
                                    <span>Discount{order.kupon ? ` (${order.kupon})` : ''}</span>
                                    <span>-{formatHarga(order.diskon)}</span>
                                </div>
                            )}
                            <div className={styles.summaryRow}>
                                <span>Shipping</span>
                                <span>{order.ongkir === 0 ? 'FREE 🎉' : formatHarga(order.ongkir)}</span>
                            </div>
                        </>
                    )}
                ...
                <div className={styles.section}>
                    <div className={styles.infoGrid}>
                        ...
                        <div className={styles.infoRow}>
                            ...
                            <span className={styles.infoValue}>{order.metodePay}</span>
                        </div>
                        {order.catatan && (
                            <div className={styles.infoRow}>
                ...
                <div className={styles.receiptFooter}>
                    <p>Thank you for shopping at <strong>Matjar Groceries</strong> 🥬</p>
                </div>
            </div>
