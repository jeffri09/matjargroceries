...
import { formatHarga, hitungDiscountPersen } from '@/lib/utils';
...
export default function ProductCard({ product }: ProductCardProps) {
    ...
    const diskonPersen = product.harga_diskon
        ? hitungDiscountPersen(product.harga, product.harga_diskon)
        : 0;
    ...
    return (
        ...
        <Link href={`/product/${product.slug}`} className={styles.cardLink}>
                ...
                <div className={styles.info}>
                    <p className={styles.category}>{product.kategori_nama || 'Product'}</p>
